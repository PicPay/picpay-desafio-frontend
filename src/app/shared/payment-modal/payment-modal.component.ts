...
import { Users } from 'src/app/payments/users-list/users'
...
export class PaymentModalComponent implements OnInit {
  valid_card: string = '1111111111111111'
  payed: boolean
  cardList = []
  paymentForm: FormGroup
  showSpinner: boolean = false
  constructor(
    @Inject(MAT_DIALOG_DATA) private modalData: Users,
  ...
  ngOnInit(): void {
    ...
    this.paymentForm = this.fb.group({
      ...
      destination_user_id: [this.modalData.id, [Validators.required]],
  ...
  onSubmit() {
    if (this.paymentForm.get('payment_card').value == this.valid_card) {
      ...
      dialogConfig.data = {
        ...
        text:
          'O pagamento não foi concluido, tente novamente selecionando um novo cartão',
      }
